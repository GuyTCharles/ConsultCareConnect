...
/* ================================
   ...
   ================================ */
function navigateFromFeatureAction(actionLabel, explicitTarget) {
    const delay = 100;
    ...
    if (!target) {
        switch (actionLabel) {
            case 'Book Appointment':
                target = 'scheduling.html';
                break;
            case 'Add or View Records':
                target = 'medical-records.html';
                break;
            case 'Start Assessment':
                target = 'symptoms-checker.html';
                break;
            default:
                console.error('Unrecognized action:', actionLabel);
                return;
        }
    }
...
// Modern full-card navigation (with keyboard support)
document.querySelectorAll('.feature-cards .card.card-action').forEach((card) => {
    const actionLabel = card.querySelector('.card-cta')?.textContent.trim() || card.textContent.trim();
    const target = card.dataset.target;

    card.addEventListener('click', (event) => {
        const clickedInteractive = event.target.closest('a, button, input, textarea, select');
        if (clickedInteractive) return;
        navigateFromFeatureAction(actionLabel, target);
    });
