...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const renderCalendar = () => {
        ...
        for (let day = 1; day <= totalDays; day++) {
            ...
            dateElement.dataset.year = String(currentYear);
            dateElement.dataset.month = String(currentMonth);
            dateElement.dataset.day = String(day);

            if (day === currentDay) {
                dateElement.classList.add("today");
            }

            if (getStartOfDay(dateValue).getTime() < todayStart.getTime()) {
...
let selectedAppointmentDate = null; // Track selected date from calendar
...
function parseConsultationTime(timeValue) {
    const match = String(timeValue || "").trim().toLowerCase().match(/^(\d{1,2})(?::(\d{2}))?\s*(am|pm)$/);
    if (!match) {
        return null;
    }
...
function buildStartDateTime(date, timeValue) {
    const parsedTime = parseConsultationTime(timeValue);
    if (!date || !parsedTime) {
        return null;
    }

    const start = new Date(date);
    start.setHours(parsedTime.hour24, parsedTime.minute, 0, 0);
    return start;
}

function updateConsultationTimeOptions() {
    const timeDropdown = document.getElementById("consultation-time");
    if (!timeDropdown) {
        return;
    }
    ...
    Array.from(timeDropdown.options).forEach((option) => {
        ...
        if (selectedAppointmentDate) {
            if (isPastCalendarDate(selectedAppointmentDate)) {
                shouldDisable = true;
            } else if (isTodayDate(selectedAppointmentDate)) {
                const optionDateTime = buildStartDateTime(selectedAppointmentDate, option.value);
                shouldDisable = !optionDateTime || optionDateTime <= now;
            }
        }
    ...
    if (timeDropdown.selectedOptions[0]?.disabled) {
        timeDropdown.value = "-";
    }
}
...
document.querySelector(".primary-button.submit-button").addEventListener("click", () => {
    ...
    const startDateTime = buildStartDateTime(selectedAppointmentDate, selectedTime);
    const now = new Date();

    if (!startDateTime) {
        alert("Please select a valid consultation time.");
        return;
    }
    ...
    if (startDateTime <= now) {
        alert("Please choose a future time for your appointment.");
        return;
    }
    ...
    const appointmentMonth = startDateTime.toLocaleString("en-US", {
        month: "long"
    });
    const appointmentDay = startDateTime.getDate();
    const appointmentYear = startDateTime.getFullYear();

    document.getElementById("confirmation-date").textContent = `${appointmentMonth} ${appointmentDay}, ${appointmentYear}`;
...
document.getElementById("consultation-time")?.addEventListener("change", () => {
    updateConsultationTimeOptions();
});
